...
def cmd_compare(args: argparse.Namespace) -> None:
    ...
    for strategy in strategies:
        ...
        training_data_summaries.append(
            _summarize_training_data(strategy, len(queries), negatives, triplets)
        )
    ...
    _write_training_data_summaries(out_dir, training_data_summaries)
    retrieval_rows = []
    for run in (
...
def _summarize_training_data(
    strategy: str,
    input_queries: int,
    negatives,
    triplets: list[dict],
) -> dict:
    negative_query_ids = {row.query_id for row in negatives}
    triplet_query_ids = {str(row["query_id"]) for row in triplets}
    return {
        "strategy": strategy,
        "input_queries": input_queries,
        "retained_negative_queries": len(negative_query_ids),
        "negative_count": len(negatives),
        "retained_triplet_queries": len(triplet_query_ids),
        "triplet_count": len(triplets),
    }


def _write_training_data_summaries(out_dir: Path, summaries: list[dict]) -> None:
    (out_dir / "training_data_summary.json").write_text(
        json.dumps(summaries, indent=2, sort_keys=True), encoding="utf-8"
    )
    fieldnames = [
        "strategy",
        "input_queries",
        "retained_negative_queries",
        "negative_count",
        "retained_triplet_queries",
        "triplet_count",
    ]
    with (out_dir / "training_data_summary.csv").open(
...
def _write_training_summaries(out_dir: Path, summaries: list[dict]) -> None:
    ...
    fieldnames = [
        "strategy", "model_name", "triplets", "epochs", "batch_size", "loss", "optimizer",
